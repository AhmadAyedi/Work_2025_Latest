...
void loop() {
    ...
    if (CAN.readMsgBuf(&canId, &len, buf)) {
        ...
        // Send corresponding LIN frame based on button state
        if (buttonState == LOW) {  // Button pressed
            linSerial.println("1:LED_ON");
            Serial.println("Sent to LIN: LED_ON");
        } else {  // Button not pressed
            linSerial.println("1:LED_OFF");
            Serial.println("Sent to LIN: LED_OFF");
        }
