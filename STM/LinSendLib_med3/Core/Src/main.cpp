...
static void MX_ITM_Init(void)
{
    // Enable ITM and SWV
    CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;  // Enable trace in debug monitor
    DBGMCU->CR |= DBGMCU_CR_TRACE_IOEN;              // Enable trace I/O
    ITM->LAR = 0xC5ACCE55;                           // Unlock ITM registers
    ITM->TCR |= ITM_TCR_ITMENA_Msk | ITM_TCR_TraceBusID_Msk; // Enable ITM and set trace bus ID
    ITM->TER |= 1 << 0;                              // Enable ITM Stimulus Port 0
}
/**
