...
uint8_t lastButtonState = 0;  // Last button state to detect change
...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	  // Read the current state of the button (PC13)
	      buttonState = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);

	      // Check for button press (transition from 1 to 0, since active-low)
	      if (buttonState == 0 && lastButtonState == 1)
	      {
	        HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5); // Toggle LED on PA5
	        HAL_Delay(50); // Simple debounce delay (50 ms)
	      }
