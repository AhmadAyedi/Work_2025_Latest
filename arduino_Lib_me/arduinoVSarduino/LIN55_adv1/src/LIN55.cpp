...
LIN55::LIN55(uint8_t rxPin, uint8_t txPin, uint8_t slpPin, uint8_t ledPin) : LIN55(rxPin, txPin, slpPin)
{
    this->ledPin = ledPin;
}
...
void LIN55::handleButtonPress()
{
    ...
    {                                                  // Button pressed (debounced)
        ...
        uint8_t message[1] = {ledState ? 0x01 : 0x00}; // Set the message to ON (0x01) or OFF (0x00)
        Serial.println(ledState ? "Sending ON Command" : "Sending OFF Command");

        sendLINFrame(0x12, message, sizeof(message)); // Send the command over LIN
